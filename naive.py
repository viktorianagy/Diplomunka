...
def generate_graph(datasetpath=" "):
    ...
    #G = nx.Graph()
    #G.add_edges_from([(0, 1), (0, 2), (1, 2), (1, 3), (1, 4), (2, 4), (3, 4)])
    fh.close()
...
def make_csv_file(name1, name2, dataset, k_hatar):
    ...
    header = ['k', 'naive', 'enum', 'max deg', 'min deg', 'G_max_rad', 'G_min_rad', 'max(c max)', 'min(c max)', 'max(c min)', 'min(c min)', 'cover']
    #header = ['k', 'max deg', 'min deg', 'G_max_rad', 'max(c max)', 'cover']
    print("h")
    ...
    while k <= m:
        ...
        row2.append(runtime)
        print('gfast:', runtime, minEigenvalue)
...
def make_csv_file_1(name1, name2, dataset, k):
    header = ['k']
    ...
    k = 1
    while mineig <= 1:
        mineig, indexes, runtime = gershgorin_fast(L, k, G, 3, 'min')
        row1.append(mineig)
        row2.append(runtime)
        k += 1
        print('gfast:', runtime, mineig)
    writer1.writerow(row1)
    writer2.writerow(row2)
    ...
    k = 1
    while mineig <= 1:
        mineig, indexes, runtime = gershgorin_fast(L, k, G, 2, 'min')
        row1.append(mineig)
        row2.append(runtime)
        k += 1
        print('gfast:', runtime, mineig)
    ...
    k = 1
    while mineig <= 1:
        indexes, mineig, runtime = ampl_run2('graf', k, G, L)
        row1.append(mineig)
        row2.append(runtime)
        k += 1
        print('cover:', runtime, mineig)
...
print(G)
print(L.toarray())
minEigenvalue, indexes, runtime = gershgorin_fast(L, 2, G, 3, 'max')
print(minEigenvalue)

minEigenvalue, indexes, runtime = gershgorin_fast(L, 2, G, 3, 'min')
print(minEigenvalue)

minEigenvalue, indexes, runtime = gershgorin_fast(L, 2, G, 2, 'max')
print(minEigenvalue)

minEigenvalue, indexes, runtime = gershgorin_fast(L, 2, G, 2, 'min')
print(minEigenvalue)

minEigenvalue, indexes, runtime = gershgorin_fast(L, 2, G, 1, 'max')
print(minEigenvalue)

minEigenvalue, indexes, runtime = gershgorin_fast(L, 2, G, 1, 'min')
print(minEigenvalue)

S, lam, runtime = naive(L, 2)
print("naive", lam, S)

mineig, ind, runtime = min_deg(L, G, 2)
print("min deg:", mineig, ind)
mineig, ind, runtime = max_deg(L, G, 2)
print("max deg:", mineig, ind, runtime)
mineig, ind, runtime = ampl_run2('graf', 2, G, L)
print("cover", mineig, ind, runtime)
mineig, ind = gap_enum(G, L, 2)
print("enum:", mineig, ind)
"""
"""
runtime_list = []
ks = []
...
plt.plot(ks, runtime_list)

# naming the x axis
plt.xlabel('k')
# naming the y axis
plt.ylabel('time (s)')
plt.scatter(ks, runtime_list, color= "black",
            marker= "o", s=30)
plt.show()
plt.savefig('20_erdos_renyi.png')

print(A.toarray())
"""
make_csv_file('tribes_eigs_enum', 'tribes_time_enum', "C:\\Diplomamunka\\ucidata-gama\\out.ucidata-gama", 4)
"""
while k <= 15:
